...
//</editor-fold>
        ...
        private void clasificarJugadores() {
        ...
        public void escribirAlineacionEnFichero() throws IOException {
        ...
        private void rellenarVacios() {
                if (porteroAli == null) {
                        if (defensa.size() > 0) {
                                porteroAli = new JugadorEnAlineacion(defensa.get(0), "GK");
                                defensa.remove(defensa.get(0));
                        } else if (dm.size() > 0) {
                                porteroAli = new JugadorEnAlineacion(dm.get(0), "GK");
                                dm.remove(dm.get(0));
                        } else if (medio.size() > 0) {
                                porteroAli = new JugadorEnAlineacion(medio.get(0), "GK");
                                medio.remove(medio.get(0));
                        } else if (am.size() > 0) {
                                porteroAli = new JugadorEnAlineacion(am.get(0), "GK");
                                am.remove(am.get(0));
                        } else if (delantero.size() > 0) {
                                porteroAli = new JugadorEnAlineacion(delantero.get(0), "GK");
                                delantero.remove(delantero.get(0));
                        }
                }
                for (JugadorEnAlineacion j : defensaAli) {
                        if (j.getJugador() == null) {
                                if (dm.size() > 0) {
                                        j = new JugadorEnAlineacion(dm.get(0), "DF");
                                        dm.remove(dm.get(0));
                                } else if (medio.size() > 0) {
                                        j = new JugadorEnAlineacion(medio.get(0), "DF");
                                        medio.remove(medio.get(0));
                                } else if (am.size() > 0) {
                                        j = new JugadorEnAlineacion(am.get(0), "DF");
                                        am.remove(am.get(0));
                                } else if (delantero.size() > 0) {
                                        j = new JugadorEnAlineacion(delantero.get(0), "DF");
                                        delantero.remove(delantero.get(0));
                                } else if (portero.size() > 0) {
                                        j = new JugadorEnAlineacion(portero.get(0), "DF");
                                        portero.remove(portero.get(0));
                                }
                        }
                }
                for (JugadorEnAlineacion j : dmAli) {
                        if (j.getJugador() == null) {
                                if (medio.size() > 0) {
                                        j = new JugadorEnAlineacion(medio.get(0), "DM");
                                        medio.remove(medio.get(0));
                                } else if (defensa.size() > 0) {
                                        j = new JugadorEnAlineacion(defensa.get(0), "DM");
                                        defensa.remove(defensa.get(0));
                                } else if (am.size() > 0) {
                                        j = new JugadorEnAlineacion(am.get(0), "DM");
                                        am.remove(am.get(0));
                                } else if (delantero.size() > 0) {
                                        j = new JugadorEnAlineacion(delantero.get(0), "DM");
                                        delantero.remove(delantero.get(0));
                                } else if (portero.size() > 0) {
                                        j = new JugadorEnAlineacion(portero.get(0), "DM");
                                        portero.remove(portero.get(0));
                                }
                        }
                ...
                for (JugadorEnAlineacion j : medioAli) {
                        if (j.getJugador() == null) {
                                if (dm.size() > 0) {
                                        j = new JugadorEnAlineacion(dm.get(0), "MF");
                                        dm.remove(dm.get(0));
                                } else if (am.size() > 0) {
                                        j = new JugadorEnAlineacion(am.get(0), "MF");
                                        am.remove(am.get(0));
                                } else if (defensa.size() > 0) {
                                        j = new JugadorEnAlineacion(defensa.get(0), "MF");
                                        defensa.remove(defensa.get(0));
                                } else if (delantero.size() > 0) {
                                        j = new JugadorEnAlineacion(delantero.get(0), "MF");
                                        delantero.remove(delantero.get(0));
                                } else if (portero.size() > 0) {
                                        j = new JugadorEnAlineacion(portero.get(0), "MF");
                                        portero.remove(portero.get(0));
                                }
                        }

                }
                for (JugadorEnAlineacion j : amAli) {
                        if (j.getJugador() == null) {
                                if (medio.size() > 0) {
                                        j = new JugadorEnAlineacion(medio.get(0), "AM");
                                        medio.remove(medio.get(0));
                                } else if (delantero.size() > 0) {
                                        j = new JugadorEnAlineacion(delantero.get(0), "AM");
                                        delantero.remove(delantero.get(0));
                                } else if (dm.size() > 0) {
                                        j = new JugadorEnAlineacion(dm.get(0), "AM");
                                        dm.remove(dm.get(0));
                                } else if (defensa.size() > 0) {
                                        j = new JugadorEnAlineacion(defensa.get(0), "AM");
                                        defensa.remove(defensa.get(0));
                                } else if (portero.size() > 0) {
                                        j = new JugadorEnAlineacion(portero.get(0), "AM");
                                        portero.remove(portero.get(0));
                                }
                ...
                for (JugadorEnAlineacion j : delanteroAli) {
                        if (j.getJugador() == null) {
                                if (am.size() > 0) {
                                        j = new JugadorEnAlineacion(am.get(0), "FW");
                                        am.remove(am.get(0));
                                } else if (medio.size() > 0) {
                                        j = new JugadorEnAlineacion(medio.get(0), "FW");
                                        medio.remove(medio.get(0));
                                } else if (dm.size() > 0) {
                                        j = new JugadorEnAlineacion(dm.get(0), "FW");
                                        dm.remove(dm.get(0));
                                } else if (defensa.size() > 0) {
                                        j = new JugadorEnAlineacion(defensa.get(0), "FW");
                                        defensa.remove(defensa.get(0));
                                } else if (portero.size() > 0) {
                                        j = new JugadorEnAlineacion(portero.get(0), "FW");
                                        portero.remove(portero.get(0));
                                }
